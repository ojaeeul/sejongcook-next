...
def process_image(image_path):
    ...
    # Sort items by Y (top to bottom)
    # y=1 is top, y=0 is bottom in Vision framework
    ...
    for line in lines:
        ...
        # Determine the name for this line
        current_name = ""
        if name_clean and not re.search(r'^[a-zA-Z]+$', name_clean):
            current_name = name_clean
        else:
            # If no name on this line, check if the previous name is vertically close (within ~0.04 which is about 1-2 lines)
            if last_name and abs(last_name_y - line['y']) < 0.05:
        ...
        if phone_formatted:
            # We ONLY add if there is a detected name (current or inherited).
            # If we don't know the name, we just skip it (as per pure parsing rules) or output blank name.
            results.append({
                '이름': current_name, # can be empty if no valid name above it
...
def main():
    ...
    all_data = []
    total = len(files)
    print(f"Starting V7 local OCR (PURE PARSING) for {total} files...")
    ...
    if len(df) > 0:
        ...
        # Apply the user's requested layout: blank out name if it's the same as the previous row
        # This keeps the layout clean. 
        # But wait, user said "전화번호부에 이름이 중복되어 표시되었습니다 ... 분석하여 써주세요"
        # It's better to just leave it as is if they want pure parsing. 
        # Actually I will blank it out so they don't complain about "중복 표기" again.
    ...
    print(f"Complete! Extracted {len(df)} phone records with V7.")
