...
function Content() {
    ...
    const fetchData = async () => {
        try {
            const res = await fetch('/api/admin/data/schedule');
            if (res.ok) {
    ...
    return (
        <div className="container_2" style={{ flexGrow: 1 }}>
            <div className="sub_title_381227_">
                <h1 className="text-2xl font-bold mb-4">자격시험일정 {isEdit && <span className="text-sm text-red-500 font-normal ml-2">- 수정 모드</span>}</h1>
            </div>
