...
function WriteForm() {
    ...
    useEffect(() => {
        if (isEdit) {
            setSubject("관련사이트 안내 (수정)");
            setAuthor("관리자");
            setContent("<p>http://example.com - 예시 사이트</p>");
        }
    }, [isEdit]);
    ...
    const handleConfirmSuccess = () => {
        window.location.href = "/community/sites";
    };

    return (
        <div className="container_2" style={{ flexGrow: 1 }}>
            <div className="sub_title_381227_">
                <h1>관련사이트 <span style={{ fontSize: '16px', fontWeight: 'normal', color: '#666' }}>- {isEdit ? "글수정" : "글쓰기"}</span></h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 mt-6">
                    <Link href="/community/sites" className="px-6 py-2 border border-gray-300 rounded hover:bg-gray-50 text-gray-700">취소</Link>
            ...
            <SuccessModal
                ...
                message={`작성하신 정보가 성공적으로 ${isEdit ? "수정" : "등록"}되었습니다.`}
            />
        </div>
    );
}

export default function SitesWritePage() {
    return (
