...
# API Cache to boost performance for many concurrent users
_api_cache = {}
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Use parsed.path to ignore query parameters (e.g., ?v=123)
        clean_path = parsed.path
        if clean_path == '/' or clean_path == '/index.html':
            self.path = '/public/index.html'
        elif not clean_path.startswith('/api') and not clean_path.startswith('/data'):
            # Check existence using the clean path
            possible_public_path = os.path.join(DIRECTORY, 'public', clean_path.lstrip('/'))
            if os.path.exists(possible_public_path):
    ...
    def _read_json(self, filename):
        global _api_cache
        if filename in _api_cache:
            return _api_cache[filename]
    ...
    def _write_json(self, filename, data):
        global _api_cache
        _api_cache.pop(filename, None) # Clear specific cache
        path = os.path.join(DATA_DIR, filename)
    ...
    def handle_save_attendance(self):
        try:
            ...
            for i, log in enumerate(logs):
                if (log['memberId'] == data['memberId'] and 
                    log['date'] == data['date'] and 
                    log.get('course') == data.get('course')):
                    existing_idx = i
                    break
        ...
        try:
            ...
            for d in dates:
                 ...
                 if existing_idx != -1:
                     ...
                     if status != 'unchecked':
                         logs.append({'memberId': memberId, 'date': d, 'status': status, 'course': data.get('course')})
            ...
            if status == 'unchecked':
                 ...
                 # Re-implementation for robustness:
                  logs = [l for l in logs if not (l['memberId'] == memberId and l['date'] in dates and l.get('course') == data.get('course'))]
    ...
    def handle_save_settings(self):
        try:
            body = self.get_body()
            data = json.loads(body)
            # data is expected to be the full settings object
            self._write_json('settings.json', data)
            self.send_response(200)
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'success': True}).encode('utf-8'))
...
# Use ThreadingTCPServer to handle many concurrent users (50+) in parallel
