...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const startStr = `${startOfCurrentMonth.getFullYear()}-${String(startOfCurrentMonth.getMonth() + 1).padStart(2, '0')}-01`;
...
function processCourses() {
    ...
    allMembers.forEach(m => {
        if (!m.course) return; // badge skips students with no course
...
function renderRangeCalendar() {
    ...
    for (let i = 1; i <= lastDate; i++) {
        ...
        // Highlight payment milestones with NAMES (Match Red/Yellow Badge Style)
