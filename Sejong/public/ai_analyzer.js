...
3. 따라서 **가장 중요한 "정답(답안지)"**를 무조건 제일 먼저 찾아내어, JSON의 "답안지" 필드에 텍스트 형식으로 추출해야 합니다.
   - 정답표가 문서 맨 끝에 모여있는 경우 그 표를 읽어서 추출하세요.
   - 만약 "정답포함" 문서처럼 각 문제 바로 밑이나 옆에 정답이 개별적으로 적혀있는 경우, 본문을 훑으며 1번부터 끝번까지의 정답을 모두 수집하여 한 곳에 정리해서 추출하세요.
...
{
    "답안지": "문서 내에 있는 모든 정답(1번부터 끝번까지)을 빠짐없이 찾아내어 텍스트로 추출하세요. (표에 있든, 각 문제 밑에 흩어져 있든 모두 모아서 '1번: 2, 2번: 1...' 형식으로 추출할 것)",
