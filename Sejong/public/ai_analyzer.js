...
window.renderCourseSettingsList = function() {
    ...
    window.allExamCourses.forEach((catObj, catIndex) => {
        ...
        // Add sub-course input form
        html += `
                    <div style="display:flex; gap:8px; margin-top: 10px; margin-left: 10px;">
                        <input type="text" id="newSubCourseInput_${catIndex}" placeholder="하위 폴더 추가" style="flex:1; padding:6px; border:1px solid #cbd5e1; border-radius:4px; font-size: 0.85rem;">
                        <button onclick="addExamCourseToCategory(${catIndex})" style="background:#10b981; color:white; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-size:0.85rem;"><i class="fas fa-plus"></i> 추가</button>
                    </div>
                </div>
            </div>
        `;
...
window.addExamCourseToCategory = function(catIndex) {
    ...
    // Check if course name exists anywhere to avoid duplicates (assuming course names are globally unique for questions)
    let exists = false;
    ...
    window.currentExamCategory = window.allExamCourses[catIndex].category;
...
window.deleteExamCategory = function(catIndex) {
    ...
    // Check if current category is deleted
    if(window.allExamCourses.length === 0) {
        window.currentExamCategory = null;
        window.currentExamCourse = null;
    } else if (!window.allExamCourses.find(c => c.category === window.currentExamCategory)) {
        window.currentExamCategory = window.allExamCourses[0].category;
        window.currentExamCourse = window.allExamCourses[0].courses.length > 0 ? window.allExamCourses[0].courses[0] : null;
    }
...
window.deleteExamCourseFromCategory = function(catIndex, courseIndex) {
    ...
    if (window.currentExamCourse === courseName) {
        const remainingCourses = window.allExamCourses[catIndex].courses;
        window.currentExamCourse = remainingCourses.length > 0 ? remainingCourses[0] : null;
    }
