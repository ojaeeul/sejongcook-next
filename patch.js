...
const targetInfoHtml = `        const infoHtml = \`
...
const newInfoHtml = `        const infoHtml = \`
            <div style="display: flex; gap: 15px; align-items: flex-start;">
                \${photoPreview}
                <div style="flex: 1; width: 100%;">
                    <div style="font-weight:700; font-size:1.1rem; color:#0f172a; margin-bottom:4px;">
...
content = content.replace(targetInfoHtml, newInfoHtml);

// Wait, we also need to make sure courseRowsHtml doesn't have a hardcoded string that breaks this.
// `courseRowsHtml` is initialized as: `let courseRowsHtml = '<div style="margin-top: 10px; display: flex; flex-direction: column; gap: 6px; width: 100%;">';`
// That's fine, it will just be inside the `courses-${m.id}` div.

fs.writeFileSync(file, content, 'utf8');
console.log('Patched infoHtml!');
