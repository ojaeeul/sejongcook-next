...
[중요 지시사항: 2~3번 교차 검증]
이미지를 한 번만 보고 넘기지 말고, 2~3번에 걸쳐서 꼼꼼히 다시 �    } else if (currentMode === 'exam') {
...
절대 단순히 나열된 순서대로 정답을 매기지 마세요! 숫자의 패턴과 블록을 논리적으로 분석하여, "몇 번 문제의 정답이 무엇인지" 세로열(Column)의 흐름을 퍼즐 맞추듯이 완벽하게 역추적해서 정확한 정답을 추출해야 합니다. `; 다음 JSON 형식의 배열로 반환하세요:
[
...
[⭐️정답표 텍스트 깨짐 주의사항⭐️]
문서(HWP 등)의 텍스트가 추출될 때, 정답표가 "가로로 4단 배열(예: 1~15, 16~30, 31~50, 51~60)"로 되어 있는 경우, 각 열의 숫자들이 가로줄 단위로 마구 섞여서 들어올 수 있습니다. (예: "1 2 3 4 5 16 17 18 19 20 31 32 33...")
절대 단순히 나열된 순서대로 정답을 매기지 마세요! 숫자의 패턴과 블록을 논리적으로 분석하여, "몇 번 문제의 정답이 무엇인지" 세로열(Column)의 흐름을 퍼즐 맞추듯이 완벽하게 역추적해서 정확한 정답을 추출해야 합니다. 
