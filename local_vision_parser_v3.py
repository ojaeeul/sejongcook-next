...
def process_image(image_path):
    ...
    # English headers to completely ignore
    ignore_pattern = r'(?i)(INDEX|Name|Address|Home|HOm|Fax|Office|HP|Phone|E-mail|Mobile)'
    ...
    for i, name_item in enumerate(names):
        ...
        # Clean name (remove english, numbers, punctuation)
        real_name = name_item['text'].strip()
        real_name = re.sub(ignore_pattern, '', real_name)
        real_name = re.sub(r'[^가-힣a-zA-Z]', '', real_name) # keep only letters
        
        # Skip if name is empty or pure english (like 'NAME' misread)
        ...
        for line in lines:
            ...
            for p_idx, phone in enumerate(phones):
                ...
                # Clean up phone number characters
                phone_clean = re.sub(r'[^\d]', '', phone_clean)
                if len(phone_clean) >= 9: # Format nicely if it's a valid length
                    ...
                    # If it's too short, just leave it as is or skip it? Let's just keep original if parsing failed
                    phone_clean = re.sub(r'\(.*?\)', '', phone.strip()).strip()
                    
                person_results.append({
                    '이름': real_name, # Name is written on every line now
                    '전화번호': phone_clean,
                    '관계': relation
                })
...
def main():
    ...
    all_data = []
    total = len(files)
    print(f"Starting V3 local OCR for {total} files using macOS Vision...")
    ...
    # 1. Remove duplicate (이름, 전화번호) pairs
    df = df.drop_duplicates(subset=['이름', '전화번호'], keep='first')
    
    # 2. Sort by Name for clean output
    ...
    print(f"Complete! Extracted {len(df)} deduplicated phone records with V3.")
