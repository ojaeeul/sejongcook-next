...
for exam_key, questions in data.items():
    for i, q in enumerate(questions):
        ...
        # 1. "정답은 X가 아니라 Y" or "X번은 오류, 정답은 Y"
        # Relaxed character limits to 150 characters to catch longer explanations between the error mention and the correction
        match1 = re.search(r'정답[^\d]*(\d)[^\d]{1,30}(아니라|오류|잘못)[^\d]{1,150}(\d)', expl)
        match2 = re.search(r'(\d)[^\d]{1,30}(아니라|오류|잘못)[^\d]{1,150}(정답|답|오답)[^\d]*(\d)', expl)
        
        # 2. "실제/올바른/진짜 정답/오답은 X번"
        match3 = re.search(r'(실제|올바른|진짜)\s*(정답|답|오답)[^\d]*(\d)', expl)
        
        # 3. "정답이 X번인 이유"
        match4 = re.search(r'(정답|답|오답)이\s*\'?(\d)\'?번인\s*이유', expl)
